...
# revision 1.0- 12/12/2016
#
# Point masscan at a subnet and this will look for open SAMBA ports,
# clean up the output, then pipe it to enum4linux.
...
def usage():
	print('MassScan to Enum4linux- python3')
	print()
	print('Usage: python3 massEnum.py -t 192.168.1.0/24 -o output.txt')
	print('-t --target	-target network')
	print('-o --output	-output file location')
	print('-h --help	-print this help file')
	print('***Requires MassScan and Enum4Linux to be installed***')
	sys.exit()
...
def enumerator(ports, target, output):
	...
	for address in oList:
		print('Running enum4linux against %s' % (address))
		arguments = ('enum4linux %s >> %s' % (address,output))
		os.system(arguments)
